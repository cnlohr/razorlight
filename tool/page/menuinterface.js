...
function onMessage(evt)
{
	...
	else
	{
		...
		{
			var wxresp = evt.data.substr(3).split("\t");
			if( Number( wxresp[0] ) == 1 ) window.close();
			output.innerHTML = "<p>Messages: " + msg + "</p><p>Online: " + wxresp[0] + "</p>"; // + " / IP: " + ((wxresp.length>1)?HexToIP( wxresp[1] ):"") + "</p>";	
		}
	}
